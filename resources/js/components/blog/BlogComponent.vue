                        ...
                        <v-expansion-panel-content>
                            <v-container>
                                ...
                                <v-row>
                                    <v-col cols="12">
                                        <span v-if="estatus == 1">
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(2)">Borrador</v-btn>
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(1)">Actualizar</v-btn>
                                        </span>
                                        <span v-else-if="estatus == 2">
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(2)">Guardar</v-btn>
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(1)">Publicar</v-btn>
                                        </span>
                                        <span v-else>
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(2)">Borrador</v-btn>
                                            <v-btn class="float-right mr-2" text outlined small color="primary" @click="blog(1)">Publicar</v-btn>
...
export default {
    ...
    methods: {
        ...
        categorias() {
            axios.get('cat/list/blog').then(response => {
                ...
                axios.post('system/logs', {
                    ...
                    file: 'components\\blog\\BlogComponent.vue'
                }).catch(error => {});
            });
        },
        ...
        guardar(estatus) {
            ...
            if (this.vmodel_titulo != '' && this.vmodel_ckeditor != '' && this.vmodel_path_destacada != null &&
                ...
                axios.post('blog/add', formData).then(response => {
                    if (response.data.answer) {
        ...
        editar(estatus) {
            ...
            if (this.vmodel_titulo != '' && this.vmodel_subtitulo != '' && this.vmodel_ckeditor != '' &&
                ...
                axios.post('blog/edit', formData).then(response => {
                    if (response.data.answer) {
                        this.$refs.form.reset();
                        this.$store.dispatch('sanckbarsMessage', [response.data.msg, 'success', true, '', ['top', 'right']]);
                        this.$router.push({
