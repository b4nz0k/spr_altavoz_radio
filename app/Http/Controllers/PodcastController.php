...
class PodcastController extends Controller
{
    ...
    public function listado($estatus)
    {
        try {
            ...
            $podcasts = '';
            if ($estatus == 'trash') {
                $podcasts = Podcast::where('estacion_radio_id', \Auth::user()->estacion_radio_id)->where('estatus', 2)->orderByDesc('created_at')->get();
            } else if ($estatus == 'remove') {
                $podcasts = Podcast::where('estacion_radio_id', \Auth::user()->estacion_radio_id)->where('estatus', 3)->orderByDesc('created_at')->get();
            } else if ($estatus == 'all'){
                $podcasts = Podcast::where('estacion_radio_id', \Auth::user()->estacion_radio_id)->where('estatus', 1)->orderByDesc('created_at')->get();
            }
